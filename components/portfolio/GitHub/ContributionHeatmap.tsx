...
function getHeatmapColor(count: number, maxCount: number): string {
  ...
  if (intensity === 1) return "bg-white/20 border-white/5";
  if (intensity === 2) return "bg-white/40 border-white/10";
  if (intensity === 3) return "bg-white/70 border-white/20";
  ...
  return (
    <div className="flex flex-col gap-10" ref={ref}>
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-6 px-2">
        <div>
          <h3 className="text-2xl font-light tracking-tight text-white mb-6 uppercase">Activity Data</h3>
          <div className="flex gap-8">
            <div className="flex flex-col">
              <span className="text-[10px] font-black tracking-[0.2em] uppercase text-muted-foreground/40 mb-1">Current Streak</span>
              <span className="text-2xl font-light text-white">{streak.current} days</span>
            </div>
            <div className="flex flex-col">
              <span className="text-[10px] font-black tracking-[0.2em] uppercase text-muted-foreground/40 mb-1">Peak Streak</span>
              <span className="text-2xl font-light text-white">{streak.longest} days</span>
            </div>
          </div>
        </div>
        <div className="flex flex-col items-end">
             <span className="text-[10px] font-black tracking-[0.2em] uppercase text-muted-foreground/40 mb-1">Total in 2025</span>
             <span className="text-5xl font-light tracking-tight text-white">{totalContributions}</span>
        </div>
      </div>

      <div className="relative rounded-[2rem] bg-white/5 p-8 md:p-10 border border-white/5 overflow-x-auto custom-scrollbar group">
        {/* Tooltip */}
