...
export default function MasonryGrid({ blogs, onTagClick }: MasonryGridProps) {
  ...
  const columnArrays = useMemo(() => {
    const arrays = Array.from({ length: columns }, () => [] as IBlog[]);
    const heights = Array(columns).fill(0);

    blogs.forEach((blog, i) => {
      const shortestColIdx = heights.indexOf(Math.min(...heights));
      arrays[shortestColIdx].push(blog);
      heights[shortestColIdx] += getWeight(blog, i);
    });
    return arrays;
  }, [blogs, columns]);

  return (
    <div className="relative p-1 rounded-[3rem] border border-border/10 bg-secondary/[0.01]">
      <div 
